// 607室增强版 - 继承BedManager，添加加床功能
class Room607Manager extends BedManager {
    constructor() {
        super('room607', room607Config);
    }
    ...
    render() {
        ...
        this.config.facilities.forEach(facility => {
            ...
            if (facility.type === 'bed-side-xray-machine') {
                facilityEl.style.backgroundColor = '#f0f8ff';
                facilityEl.style.border = '1px solid #4169e1';
                facilityEl.style.display = 'flex';
                facilityEl.style.justifyContent = 'center';
                facilityEl.style.alignItems = 'center';
                facilityEl.style.fontSize = '9px';
                facilityEl.style.fontWeight = 'bold';
                facilityEl.style.color = '#4169e1';
                facilityEl.style.textAlign = 'center';
            }
            
            if (facility.type === 'small-bed-collection-area') {
                facilityEl.style.backgroundColor = '#f5f5dc';
                facilityEl.style.border = '1px solid #daa520';
                facilityEl.style.display = 'flex';
                facilityEl.style.justifyContent = 'center';
                facilityEl.style.alignItems = 'center';
                facilityEl.style.fontSize = '9px';
                facilityEl.style.fontWeight = 'bold';
                facilityEl.style.color = '#daa520';
                facilityEl.style.textAlign = 'center';
            }
        ...
        allBeds.forEach(bed => {
            ...
            if (bed.type === 'infection-bed') {
                ...
                bedEl.style.backgroundColor = '#fff2e8';
                bedEl.style.borderColor = '#fa8c16';
            }
    ...
    updateTitleInfo() {
        ...
        // 计算感染病区床位数
        const infectionBeds = this.config.beds.filter(bed => bed.type === 'infection-bed').length;
        ...
        if (infectionBeds > 0) {
            additionalInfo.push(`含${infectionBeds}张感染病区床位`);
        }
        
        const additionalBedInfo = additionalInfo.length > 0 ? ` (${additionalInfo.join(', ')})` : '';
        
        titleEl.innerHTML = `
            <span id="current-date">${year}年${month}月${day}日</span>607室床位分布图${additionalBedInfo}, 
            总床位${totalBeds}张,
        ...
        ['patient-count', 'discharge-count'].forEach(id => {
            ...
            if (inputElement) {
                inputElement.addEventListener('change', function() {
                    ...
                    localStorage.setItem('room607Counts', JSON.stringify(counts));
                });
            }
        });
        ...
        // 加载保存的计数值
        try {
            const savedCounts = localStorage.getItem('room607Counts');
            if (savedCounts) {
...
// 创建全局实例
let room607Manager = null;

// 初始化函数
function initRoom607Enhanced() {
    room607Manager = new Room607Manager();
    room607Manager.render();
}
...
function openBedModal(bedId) {
    if (room607Manager) {
        room607Manager.openBedModal(bedId);
    }
}
