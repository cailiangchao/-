// 605室增强版 - 继承BedManager，添加加床功能
class Room605Manager extends BedManager {
    constructor() {
        super('room605', room605Config);
        
        // 处理现有的"加床"，将其转换为additionalBeds
        this.migrateExistingAdditionalBeds();
        
        // 处理隔离病房
        this.initIsolationWards();
    }
    ...
    initIsolationWards() {
        this.config.facilities
            ...
            .forEach(ward => {
                if (!this.bedData[ward.id]) {
                    this.bedData[ward.id] = {
                        patientName: '',
                        status: 'empty', 
                        remarks: ''
                    };
    ...
    render() {
        ...
        this.config.facilities.forEach(facility => {
            ...
            // 不渲染隔离病房，因为它们会作为床位单独处理
            if (!facility.isIsolation) {
        ...
        this.config.facilities
            ...
            .forEach(ward => {
                ...
                wardEl.className = 'bed isolation-ward';
                wardEl.id = `bed-${ward.id}`;
                wardEl.style.left = `${ward.position.x}px`;
                wardEl.style.top = `${ward.position.y}px`;
                wardEl.style.width = `${ward.width}px`;
                wardEl.style.height = `${ward.height}px`;
                wardEl.style.transform = `rotate(${ward.rotation || 0}deg)`;
                wardEl.style.transformOrigin = 'center center';
                
                // 隔离病房特殊样式
                wardEl.style.border = '3px solid #ff6b6b';
                wardEl.style.backgroundColor = '#ffe0e0';
                
                // 床位内容
                const wardData = this.bedData[ward.id] || {};
                const hasPatient = wardData.patientName && wardData.patientName.trim() !== '';
                
                let currentStatus = wardData.status || 'empty';
                if (hasPatient && currentStatus === 'empty') {
                    currentStatus = 'occupied';
                    this.bedData[ward.id].status = 'occupied';
                } else if (!hasPatient && (currentStatus === 'occupied' || currentStatus === 'discharge_planned')) {
                    currentStatus = 'empty';
                    this.bedData[ward.id].status = 'empty';
                }
                
                wardEl.classList.add(currentStatus);
                
                wardEl.innerHTML = `
                    <div class="bed-label">${ward.id}</div>
                    <div class="patient-area">${hasPatient ? wardData.patientName : ''}</div>
                `;
                
                // 点击事件
                wardEl.addEventListener('click', () => this.openBedModal(ward.id));
    ...
    // 重写获取床位总数，包括隔离病房
    getTotalBedCount() {
        const isolationWards = this.config.facilities.filter(f => f.isIsolation).length;
        return this.config.beds.length + this.additionalBeds.length + isolationWards;
    }
    ...
    updateTitleInfo() {
        ...
        const isolationWardCount = this.config.facilities.filter(f => f.isIsolation).length;
        const additionalInfo = [];
        
        if (additionalBedCount > 0) {
            additionalInfo.push(`含${additionalBedCount}张加床`);
        }
        if (isolationWardCount > 0) {
            additionalInfo.push(`含${isolationWardCount}间隔离病房`);
        }
        
        const additionalBedInfo = additionalInfo.length > 0 ? ` (${additionalInfo.join(', ')})` : '';
        
        titleEl.innerHTML = `
            <span id="current-date">${year}年${month}月${day}日</span>605室床位分布图${additionalBedInfo}, 
            总床位${totalBeds}张,
        ...
        ['patient-count', 'discharge-count'].forEach(id => {
            ...
            if (inputElement) {
                inputElement.addEventListener('change', function() {
                    ...
                    localStorage.setItem('room605Counts', JSON.stringify(counts));
                });
            }
        });
        ...
        // 加载保存的计数值
        try {
            const savedCounts = localStorage.getItem('room605Counts');
            if (savedCounts) {
...
// 创建全局实例
let room605Manager = null;

// 初始化函数
function initRoom605Enhanced() {
    room605Manager = new Room605Manager();
    room605Manager.render();
}
...
function openBedModal(bedId) {
    if (room605Manager) {
        room605Manager.openBedModal(bedId);
    }
}
